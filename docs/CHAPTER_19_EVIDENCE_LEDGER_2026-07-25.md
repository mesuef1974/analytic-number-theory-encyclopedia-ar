...
\[
\psi(x+h)-\psi(x)=\sum_{x<n\le x+h}\Lambda(n).
\]
...
الانتقال من \(\psi\) إلى \(\theta\) أو إلى عدد الأوليات يتطلب حدًا مستقلًا لمساهمة \(p^k\) مع \(k\ge2\). لا يجوز استنتاج وجود أولي من إيجابية فرق \(\psi\) قبل ضبط هذه المساهمة بالنسبة إلى \(h\).
...
- استخراج نصوص المبرهنات الأصلية ومعاملاتها وكمّيات \(\varepsilon\).
