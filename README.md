...
- الفصل التاسع بحالة `DRAFT` بعد إغلاق بوابة `EVIDENCE-FIRST / PRE-AUTHORING` وكتابة النواة البرهانية على Draft PR #13.
- يثبت مسار الفصل التاسع المخطط \(\psi(x)\sim x\)، ثم يستنتج \(\vartheta(x)\sim x\) و\(\pi(x)\sim x/\log x\)، مع وسم Wiener--Ikehara بنتيجة `CITED` وإبقاء النتائج الداخلية `DRAFT` حتى التدقيق.
- المسار الكلاسيكي الفعال مفصول عن المسار التاوبيري، وتبقى ديون بيرون وتحويل المسار معلنة.
...
الفصول الحالية متفاوتة الاكتمال، ولا يعد أي فصل `RELEASE-READY` قبل اجتياز بوابة الجودة والمراجعة الثانية وإغلاق ديونه المعلنة. الفصل الثامن باقٍ `REVIEWED` ولا يدعي هذا الإصدار أنه `RELEASE-READY`. والفصل التاسع ما يزال `DRAFT` وغير جاهز للدمج.
