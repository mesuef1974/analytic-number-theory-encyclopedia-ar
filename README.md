...
- بدأت المرحلة الصفرية للفصل العاشر عن مبرهنة الأعداد الأولية في المتتاليات الحسابية على Draft PR #15.
- حالة الفصل العاشر الحالية `EVIDENCE-FIRST / PRE-AUTHORING`، وبوابة التأليف مفتوحة لكن التأليف `BLOCKED`؛ لم يُنشأ ملف الفصل ولم يُربط بالكتاب.
- الهدف النوعي المقترح للترديد الثابت هو \(\psi(x;q,a)\sim x/\varphi(q)\)، ثم \(\vartheta(x;q,a)\sim x/\varphi(q)\)، ثم \(\pi(x;q,a)\sim x/(\varphi(q)\log x)\).
...
الفصول الحالية متفاوتة الاكتمال، ولا يعد أي فصل `RELEASE-READY` قبل اجتياز بوابة الجودة والمراجعة الثانية وإغلاق ديونه المعلنة. الفصلان الثامن والتاسع بحالة `REVIEWED`. الفصل العاشر لم يدخل التأليف بعد، وPR #15 Draft وغير مدمج.
