...
- المرحلة الصفرية للفصل التاسع مفتوحة بمنهج `EVIDENCE-FIRST / PRE-AUTHORING`؛ أُنشئ سجل الأدلة وخريطة الاعتماد، ولم تبدأ كتابة متن الفصل.
- الهدف المركزي المخطط للفصل التاسع هو \(\psi(x)\sim x\)، ثم \(\vartheta(x)\sim x\) و\(\pi(x)\sim x/\log x\).
...
الفصول الحالية متفاوتة الاكتمال، ولا يعد أي فصل `RELEASE-READY` قبل اجتياز بوابة الجودة والمراجعة الثانية وإغلاق ديونه المعلنة. الفصل الثامن باقٍ `REVIEWED` ولا يدعي هذا الإصدار أنه `RELEASE-READY`.
