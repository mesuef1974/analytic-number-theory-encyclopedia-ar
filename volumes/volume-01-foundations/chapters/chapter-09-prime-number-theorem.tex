...
\noindent\textbf{حالة الفصل:} \textenglish{DRAFT}. يثبت هذا الفصل الصيغة
المركزية
\[
\psi(x)\sim x,
\]
...
\textenglish{EVIDENCE-FIRST / PRE-AUTHORING} في
\texttt{docs/CHAPTER\_09\_PRE\_AUTHORING\_AUDIT\_2026-07-20.md}.
ولا تعني حالة \textenglish{DRAFT} أن الفصل \textenglish{VERIFIED} أو
...
\begin{enumerate}
  ...
  \item إثبات عدم انعدام \(\zeta(s)\) على الخط \(\Re(s)=1\) من دون دور منطقي.
...
-\frac{\zeta'}{\zeta}(s).
\]
التقارب المطلق صحيح في \(\Re(s)>1\). وبالتكامل بالتجزئة على
\([1,X]\):
\[
...
أثبت الفصل السابع المتراجحة المثلثية
\[
3+4\cos u+\cos2u
=2(1+\cos u)^2\ge0.
\]
...
`ANT-LEM-07-01`، فتنتج الدعوى.
\end{proof}

\section{عدم انعدام زيتا على الخط \(\Re(s)=1\)}

\begin{theorem}[مبرهنة Hadamard--de la Vallée Poussin النوعية]
\label{thm:zeta-nonvanishing-on-one-line}
\resultid{ANT-THM-09-01}
\provedhere
لا تنعدم دالة زيتا على الخط \(\Re(s)=1\):
\[
\zeta(1+it)\ne0
\qquad(t\in\mathbb R).
\]
عند \(t=0\) تفهم العبارة بأن النقطة \(s=1\) قطب بسيط لا صفر.
\end{theorem}

\begin{proof}
يكفي معالجة \(t\ne0\). افترض عكس المطلوب، ولتكن رتبة الصفر عند
\(1+it\) مساوية \(m\ge1\). ولتكن \(m_2\ge0\) رتبة الصفر عند
\(1+2it\)، مع \(m_2=0\) إذا لم تكن تلك النقطة صفرًا.
...
المتراجحة. إذن لا يوجد الصفر المفترض.
\end{proof}
...
\qquad a_n\ge0,
\]
متقاربة في \(\Re(s)>1\)، وضع
\[
S(x)=\sum_{n\le x}a_n.
\]
افترض أن \(S(n)=O(n)\)، وأن
\[
f(s)-\frac{A}{s-1}
\]
...
من `ANT-THM-06-01` توجد دالة \(h\) هولومورفية قرب \(1\)، تحقق
\[
\zeta(s)=\frac{h(s)}{s-1},
\qquad h(1)=1.
\]
...
إذن الباقي بعد طرح \(1/(s-1)\) يمتد تحليليًا على الخط كله.
...
f(s)-\frac1{s-1}
\]
يمتد تحليليًا إلى نصف المستوى المغلق \(\Re(s)\ge1\). لذلك تعطي المبرهنة
مع \(A=1\):
\[
\psi(n)\sim n.
\]
...
\begin{proof}
هذا تطبيق مباشر لـ`ANT-THM-02-04` على النتيجة
\(\vartheta(x)\sim x\).
\end{proof}
...
المسار الكلاسيكي الفعال يحتاج سلسلة إضافية:
...
في حالة المستودع الحالية، المنطقة الكلاسيكية الخالية من الأصفار مسجلة
`CITED` في `ANT-THM-06-09`، والصيغة الصريحة مسجلة `CITED` في
`ANT-THM-06-08`، أما بيرون وتحويل المسار فبراهينهما التفصيلية مؤجلة. لذلك
لا ننسب إلى الفصل حد خطأ فعالًا.
...
\begin{enumerate}
  ...
  \item استعمال المنطقة الكمية الخالية من الأصفار في المسار النوعي من غير
  حاجة، ثم إخفاء اعتمادها المرجعي.
  \item تطبيق Wiener--Ikehara من دون التحقق من عدم سلبية المعاملات وحد
  المجاميع الجزئية.
  \item تقدير القوى الأولية العليا باستعمال PNT نفسها.
  \item اعتبار الصيغة الصريحة `PROVED-HERE` مع أن حالتها في الفصل السادس
  `CITED`.
...
\begin{enumerate}
  ...
  `ANT-PROP-09-01` بالتكامل بالتجزئة لستيلتج.
\end{enumerate}
...
المرحلة التالية ليست الدمج، بل تشغيل البناء وفحوص الجودة، ثم تدقيق منطقي
مستقل عن جولة التأليف، وتحقق مرجعي، ثم مراجعة ثانية مستقلة.
...
إضافية لم ندع اكتمالها في هذه المسودة.