...
\noindent\textbf{حالة الفصل:} \textenglish{DRAFT}. يثبت هذا الفصل أن كل
...
P_{q,a}(\sigma)
=
\sum_{\substack{p\ \mathrm{أولي}\\ p\equiv a\, (\mathrm{mod}\,q)}}
\frac1{p^\sigma},
\qquad \sigma>1.
\]
...
\begin{proof}
من المبرهنة \ref{prop:dirichlet-prime-class-filter} ومن تعريف
\(H_\chi\):
\begin{align*}
P_{q,a}(\sigma)
&=
...
\end{align*}
يفصل حد الشخصية الرئيسية عن بقية الحدود. بحسب المبرهنة
\ref{prop:principal-character-log-growth} يساوي حدها
\[
\frac1{\varphi(q)}
\log\frac1{\sigma-1}+O_q(1).
\]
وبحسب المبرهنة \ref{prop:nonprincipal-character-log-bounded} تبقى حدود
...
بترديد \(q\) منته، فتنتج الصيغة التقاربية المعلنة.
