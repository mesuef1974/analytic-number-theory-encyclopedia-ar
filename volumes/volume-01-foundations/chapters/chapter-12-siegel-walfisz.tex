...
\noindent\textbf{حالة الفصل:} \textenglish{VERIFIED}. يرقّي هذا الفصل نتيجة
...
\texttt{docs/CHAPTER\_12\_PRE\_AUTHORING\_AUDIT\_2026-07-20.md}، ثم اجتاز
الفصل تدقيق ما بعد التأليف بحكم \textenglish{PASS} ونجح في فحص الجودة
وبناء PDF الكامل؛ انظر
\texttt{docs/CHAPTER\_12\_AUTHORING\_AUDIT\_2026-07-20.md}. وتوجد خريطة
البرهان وتدقيق الديون التحليلية في:
\begin{itemize}
  \item \texttt{research/literature-reviews/chapter-12-siegel-walfisz-evidence.md}.
  \item \texttt{research/literature-reviews/chapter-12-siegel-walfisz-proof-map.md}.
  \item \texttt{docs/CHAPTER\_12\_PERRON\_CONTOUR\_EXPLICIT\_FORMULA\_AUDIT\_2026-07-20.md}.
  \item \texttt{docs/CHAPTER\_12\_EXCEPTIONAL\_ZERO\_INEFFECTIVITY\_AUDIT\_2026-07-20.md}.
  \item \texttt{docs/CHAPTER\_12\_LOGIC\_AUDIT\_2026-07-20.md}.
...
مبرهنة Bombieri--Vinogradov أو Linnik أو فرضية ريمان المعممة. ولا تعني
...
\begin{theorem}[الصيغة الصريحة المقطوعة بعد عزل الاستثناء]
\label{thm:chapter12-truncated-explicit-formula}
\resultid{ANT-THM-12-02}
\citedresult{\cite{Davenport2000,MontgomeryVaughan2007,IwaniecKowalski2004}}
توجد ثوابت مطلقة \(c,C>0\) بالخاصية الآتية. لتكن \(\chi\) شخصية بدائية
غير رئيسية بموصل \(r\)، ولتكن \(x\ge3\) و\(r\le T\le x\). يمكن اختيار
\(T_0\in[T,2T]\) لا يساوي ارتفاع صفر، بحيث
\[
\psi_0(x,\chi)
=
-\mathbf 1_{\beta_\chi}
...
+O\!\left(
xe^{-\frac{c\log x}{\log(r(T+2))}}\log^2(r(T+2))
+
\frac{x\log^2(rxT)}{T}
+
\log^2(rxT)
...
الصيغة \((12.3)\) هي نتيجة قياسية للصيغة الصريحة المقطوعة، وتقدير عد
الأصفار، والمنطقة القياسية الخالية. وقد ضمت مساهمات الأصفار البديهية وعامل
...
ومن ثم
\[
\exp\!\left(-\frac{cL}{\log(r(T+2))}\right)
...
\]
نعوض في \((12.12)\). تجمع الحدود الرئيسية إلى
\(\operatorname{Li}(x)/\varphi(q)\)، حتى ثابت محدود يمتص في الخطأ. أما
الخطأ الطرفي والتكامل، فبعد تقسيم المجال عند \(\sqrt x\) عند الحاجة، يحققان
\[
\frac{E(x)}{\log x}
+
