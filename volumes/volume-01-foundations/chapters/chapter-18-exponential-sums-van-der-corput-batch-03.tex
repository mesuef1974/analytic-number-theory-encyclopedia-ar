...
\symbolindex{الأزواج الأسية: لغة لضغط التقديرات}
...
\begin{tikzpicture}[scale=4.2]
  \draw[->] (-0.05,0) -- (0.65,0) node[right,font=\scriptsize] {$\kappa$};
  \draw[->] (0,-0.05) -- (0,1.15) node[above,font=\scriptsize] {$\lambda$};
  \fill[blue!18] (0,0.5) -- (0,1) -- (0.5,0.5) -- cycle;
  \draw[thick,blue!60!black] (0,0.5) -- (0,1) -- (0.5,0.5) -- cycle;
  \draw[dashed,gray] (0,0.5) -- (0.6,0.5);
  \draw[dashed,gray] (0.5,0) -- (0.5,0.55);
  \node[font=\scriptsize] at (-0.06,0.5) {$\frac12$};
  \node[font=\scriptsize] at (-0.06,1) {$1$};
  \node[font=\scriptsize] at (0.5,-0.06) {$\frac12$};
  \filldraw[black] (0,1) circle (0.4pt) node[above right,font=\scriptsize] {$(0,1)$};
  \filldraw[black] (0.5,0.5) circle (0.4pt) node[below right,font=\scriptsize] {$\left(\frac12,\frac12\right)$};
\end{tikzpicture}
\caption{إغلاق منطقة الأزواج الأسية \((\kappa,\lambda)\) من
التعريف~\ref{def:exponent-pair-framework}: المثلث المفتوح
\(\{0\le\kappa\le\frac12\le\lambda\le1,\ \kappa+\lambda<1\}\) مع نقطتيه
الحديّتين المصرَّح بهما صراحةً \((0,1)\) و\(\left(\frac12,\frac12\right)\).
الحد المائل \(\kappa+\lambda=1\) غير محتوًى فعليًا إلا عند هاتين النقطتين؛
التظليل يمثل الإغلاق لغرض العرض لا المجموعة المفتوحة بحرفيتها.}
\label{fig:exponent-pair-region}
\end{antfigure}

\section{عملية \(A\): التفريق وخفض درجة الطور}
\symbolindex{عملية \(A\): التفريق وخفض درجة الطور}
...
\theoremindex{عملية \(A\) في النسخة المحدودة}
...
\symbolindex{عملية \(B\): التحويل الثنائي والطور الساكن}
...
\theoremindex{عملية \(B\) في إطار الأزواج الأسية}
...
\symbolindex{تركيب العمليتين وحدود الاستخدام}
