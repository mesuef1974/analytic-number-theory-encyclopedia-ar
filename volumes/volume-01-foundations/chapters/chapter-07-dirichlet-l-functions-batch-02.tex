...
\symbolindex{موصل الشخصية}
\symbolindex{الشخصية البدائية}
...
\]
ولإثبات الشمول، خذ فئة \(u\bmod d\) أولية مع \(d\). لكل أولي
\(p\mid q\) ولا يقسم \(d\)، افرض الشرط
\[
n\equiv1\pmod{p^{v_p(q)}},
\]
مع الشرط \(n\equiv u\pmod d\). وهذه التوافقات متّسقة لأن \(d\) أولي مع
حاصل ضرب القوى الأولية المقابلة للأوليات التي لا تقسمه، فتنتج مبرهنة
الباقي الصيني عددًا \(n\). إذا كان \(p\mid d\)، فإن
\(n\equiv u\pmod d\) يضمن \(p\nmid n\)، وإذا كان \(p\nmid d\) فقد
فرضنا \(n\equiv1\pmod p\). لذلك \((n,q)=1\)، وصورة فئته بترديد \(q\)
هي الفئة \(u\bmod d\).
...
\theoremindex{الجد البدائي وموصل شخصية ديريشليه}
...
\personindex{غاوس، كارل فريدريش}
\symbolindex{مجموع غاوس \(\tau(\chi)\)}
\symbolindex{تحويل فورييه المنتهي}
...
\theoremindex{فصل مجموع غاوس للشخصية البدائية}
...
\theoremindex{صيغة بواسون على فئة حسابية}
...
\symbolindex{دالة \(L\) المكتملة \(\Lambda(s,\chi)\)}
...
\theoremindex{الاستمرار العالمي والمعادلة الوظيفية لدالة \(L\)}
...
\]
نقسم التكامل عند \(t=1\). وفي الجزء \(0<t<1\) نستعمل تحويل
...
\theoremindex{الأصفار البديهية والشريط الحرج لدوال ديريشليه \(L\)}
...
\qquad(n=0,1,2,\ldots).
\]
وجميع هذه الأصفار بسيطة: في الحالة الفردية \(a=1\) لكل \(n\geq0\)،
وفي الحالة الزوجية \(a=0\) لكل \(n\geq0\)، بما في ذلك الصفر عند
\(s=0\)، لأن المبرهنة \ref{thm:dirichlet-l-nonvanishing-at-one} تعطي
\(L(1,\overline\chi)\ne0\).
...
يلغي \(L\) قطب غاما البسيط بصفر بسيط بالضبط. أما في الحالة
\(a=0,n=0\)، فيقابل الموضع \(s=1\)، وقد أثبت عدم انعدام
\(L(1,\overline\chi)\)، فتنتج البساطة بالطريقة نفسها.
...
\symbolindex{دالة بيتا لديريشليه}
...
\begin{enumerate}
  ...
  (\mathbb Z/d\mathbb Z)^{\times}\) عندما \(d\mid q\)، ثم قارن برهانك
  بالبناء الصريح السابق عبر مبرهنة الباقي الصيني.
...
\section{ما بقي قبل اعتماد الفصل السابع}

أُغلقت النواة البرهانية المقررة للفصل، بما فيها عدم الانعدام عند
\(s=1\). وتبقى قبل تغيير حالة الفصل:

\begin{itemize}
  \item إتمام التمرير التحريري وتوحيد العبارات بين الدفعات.
  \item مطابقة الصيغ والمراجع مطابقة مستقلة، ولا سيما عوامل غاما وجذر العدد
  وتحويل ثيتا.
  \item نجاح فحوص الجودة وإعادة بناء ملف PDF بعد التصحيحات.
  \item مراجعة ثانية مستقلة قبل رفع الفصل إلى \textenglish{REVIEWED}.
\end{itemize}

أما استعمال التعامد والمشتقة اللوغاريتمية وعدم الانعدام لإثبات مبرهنة
