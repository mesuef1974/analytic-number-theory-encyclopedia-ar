...
\noindent\textbf{حالة الفصل:} \textenglish{VERIFIED}. طوبقت فروض النتائج
وبراهينها مع سجل المعرفات، ونجحت فحوص الجودة والبناء. لا تعني هذه البوابة
أن الفصل اجتاز مراجعة ثانية مستقلة.

يفترض الفصل الإلمام بالنهايات والتكامل والتكامل بالتجزئة. بنهايته ينبغي
أن يصرّح القارئ باعتماد الثوابت الضمنية، ويميز بين \(O\) و\(o\)
و\(\sim\)، ويثبت صيغتي الجمع الجزئي، وينقل تقديرًا لدالة مجاميع جزئية
إلى مجموع موزون. الصيغ القياسية والتطبيقات الإضافية يمكن مراجعتها في
\cite{Apostol1976,Davenport2000}.
...
\symbolindex{رمز لاندو الكبير \(O\)}
...
\symbolindex{رمز لاندو الصغير \(o\)}

\begin{definition}
إذا كانت \(g(x)\neq0\) لكل \(x\) كبير بما يكفي، نكتب
\[
f(x)=o(g(x))
\qquad (x\to\infty)
\]
...
\symbolindex{التكافؤ التقاربي \(\sim\)}
...
\begin{proposition}
\label{prop:asymptotic-equivalence-transitive}
\resultid{ANT-PROP-02-02}
\provedhere
...
\begin{proposition}
\label{prop:logs-preserve-asymptotic-equivalence}
\resultid{ANT-PROP-02-01}
\provedhere
إذا كانت \(f(x)>0\) و\(g(x)>0\) في النهاية، وكان
\[
f(x)\sim g(x)
\qquad\text{و}\qquad
g(x)\to\infty,
...
=
1+
\frac{\log(f(x)/g(x))}{\log g(x)}.
\]
وبما أن \(f(x)/g(x)\to1\)، فإن \(\log(f(x)/g(x))\to0\)، بينما
\(\log g(x)\to\infty\). ومن ثم يؤول الحد الثاني إلى الصفر.
\end{proof}
...
\begin{proposition}
\label{prop:big-o-arithmetic}
\resultid{ANT-PROP-02-03}
\provedhere
...
\begin{proposition}
\label{prop:little-o-times-big-o}
\resultid{ANT-PROP-02-04}
\provedhere
إذا كان \(f=o(g)\) و\(h=O(k)\)، حيث لا تنعدم \(g\) و\(k\) في
النهاية، فإن
\[
fh=o(gk).
\]
...
\begin{theorem}
\label{thm:log-powers-below-powers}
\resultid{ANT-THM-02-03}
\provedhere
...
\theoremindex{صيغة الجمع الجزئي}
\label{thm:discrete-partial-summation}
\resultid{ANT-THM-02-01}
\provedhere
...
\personindex{أبيل، نيلس هنريك}

\begin{theorem}[الجمع الجزئي لأبيل]
\theoremindex{صيغة الجمع الجزئي لأبيل}
\label{thm:abel-partial-summation}
\resultid{ANT-THM-02-02}
\provedhere
لتكن
\[
...
\begin{proof}
الدالة \(A(t)\) درجية، وتقفز بمقدار \(a_n\) عند كل عدد صحيح \(n\). لذلك يمكن كتابة المجموع كتكامل ستيلتج:
\[
\sum_{n\leq x}a_nf(n)
=
\int_{1^-}^{x}f(t)\,dA(t).
...
\]
حيث \(\gamma\) ثابت أويلر؛ انظر مثلًا \cite{Apostol1976}.
...
A(t)=\sum_{n\leq t}a_n,
\]
وبذلك تكون \(A\) دالة درجية متصلة من اليمين. وعند نقاط القفز يحدد هذا
الاصطلاح بدقة أي الحدود الطرفية تدخل في المجموع.
...
\]
والنتيجة التالية تعزل خطوة النقل ولا تفترض برهان التقدير
\(\vartheta(x)\sim x\) نفسه.

\begin{theorem}
\label{thm:theta-implies-pnt}
\resultid{ANT-THM-02-04}
\provedhere
إذا كان
\[
\vartheta(x)\sim x,
...
\begin{proof}
اكتب \(\vartheta(t)=t+r(t)\)، حيث \(r(t)=o(t)\). الحد الطرفي يحقق
\[
\frac{\vartheta(x)}{\log x}
=
\frac{x}{\log x}\bigl(1+o(1)\bigr).
\]
أما حد التكامل فإن
\[
\int_2^x\frac{\vartheta(t)}{t(\log t)^2}\,dt
...
\]
ولكل \(\varepsilon>0\) يكون \(|r(t)|\leq\varepsilon t\) بعد عتبة
مناسبة؛ لذا يعطي التقسيم نفسه، مع بقاء الجزء الابتدائي ثابتًا، أن التكامل
الثاني \(o(x/\log x)\). بالتعويض في صيغة الجمع الجزئي تنتج الدعوى.
\end{proof}

لا يثبت هذا الفصل الفرضية \(\vartheta(x)\sim x\)؛ فذلك هو الجزء
العميق المؤجل إلى فصل مبرهنة الأعداد الأولية.
