...
docs/CHAPTER_58_PROOF_MAP_2026-07-29.md:
  ...
  (3) Fourier coefficients of g in closed form:
          int_0^1 e(jx) g(x) dx = -1/j  for j != 0,  and = 0 for j = 0.
      Computed against the CLOSED FORM i.pi.(1-2x), so the proof never has
      to interchange the integral with the conditionally convergent series.
  (4) the integral representation (the pivotal step)
          int_0^1 |f(x)|^2 g(x) dx = sum_{m != n} z_m conj(z_n)/(n-m),
      where f(x) = sum_{n=1}^{N} z_n e(nx)  (a FINITE sum)
  (5) Hilbert's inequality with Schur's sharp constant pi
  (6) the form vanishes identically for real coefficients
...
def main():
    ...
    print(f"(2) g(x) = i.pi.(1-2x)   max abs err = {err2:.2e}"
          f"   |g| < pi: {bool(np.all(np.abs(g_exact) < np.pi))}")

    # (3) Fourier coefficients of g from the closed form: int e(jx) g(x) dx = -1/j
    nodes3 = 2_000_000
    ...
    print(f"(3) Fourier coeffs of g  max |int e(jx)g - (-1/j)| = {err3:.2e}"
          f"   (j=0 case: {err3_zero:.1e})")

    print("(4) integral representation (pivotal step):")
    worst_identity = 0.0
    ...
    print("(6) vanishing for real coefficients (antisymmetric kernel):")
    worst_real = 0.0
    for _ in range(5):
    ...
    ok = (err1 < 1e-4 and err2 < 1e-4 and err3 < 1e-9 and err3_zero < 1e-9
          and worst_identity < 1e-8 and worst_ratio <= 1.0 and worst_real < 1e-10)
